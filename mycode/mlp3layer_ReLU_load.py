...
def test_mlp(learning_rate=0.01, L1_reg=0.00, L2_reg=0.0001, n_epochs=1000,
             dataset='mnist.pkl.gz', batch_size=20, hidden_layer_sizes=[100,100,100], seed=1234, model='../model/3layerReLUmodel-100-100-100-seed-7.dat'):
    """
   ...
   """
    ...
    '''
    act1 = ReLU(T.dot(test_set_x, classifier.params[2]) + classifier.params[3])
    act2 = ReLU(T.dot(act1, classifier.params[4]) + classifier.params[5])
    act3 = ReLU(T.dot(act2, classifier.params[6]) + classifier.params[7])
    ...
    print('----activation1----')
    act1 = ReLU(T.dot(valid_set_x, classifier.params[2]) + classifier.params[3])
    act2 = ReLU(T.dot(act1, classifier.params[4]) + classifier.params[5])
    act3 = ReLU(T.dot(act2, classifier.params[6]) + classifier.params[7])
    act4 = T.nnet.softmax(T.dot(act3, classifier.params[0]) + classifier.params[1])
    ...
    m_act1 = ReLU(T.dot(test_set_x, classifier.params[2]) + classifier.params[3]).eval()
    for threshold in range(76, 510, 1000): # 192~622
        threshold /= 1000.
        m_act1[:,stdev_act1 < threshold] = 0
        m_act2 = ReLU(T.dot(m_act1, classifier.params[4]) + classifier.params[5])
        m_act3 = ReLU(T.dot(m_act2, classifier.params[6]) + classifier.params[7])
    ...
    m_act1 = ReLU(T.dot(test_set_x, classifier.params[2]) + classifier.params[3])
    m_act2 = ReLU(T.dot(m_act1, classifier.params[4]) + classifier.params[5]).eval()
    for threshold in range(0, 650, 2000): #745
        threshold /= 1000.
        m_act2[:,stdev_act2 < threshold] = 0
        m_act3 = ReLU(T.dot(m_act2, classifier.params[6]) + classifier.params[7]).eval()
        m_act4 = T.nnet.softmax(T.dot(m_act3, classifier.params[0]) + classifier.params[1])
    ...
    m_act2 = ReLU(T.dot(m_act1, classifier.params[4]) + classifier.params[5]).eval()
    m_act3 = ReLU(T.dot(m_act2, classifier.params[6]) + classifier.params[7]).eval()
    for threshold in range(0, 1450, 20):
        threshold /= 1000.
