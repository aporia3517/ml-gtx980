...
class HiddenLayer(object):
    ...
    def errors(self, y):
        ...
        if y.dtype.startswith('float'):
            ...
            return T.sum(T.abs_(self.y_pred - y) >= 0.2)
...
class MLP(object):
    ...
    def __init__(self, rng, input, n_in, n_hidden, n_out):
        ...
        self.negative_log_likelihood = (
            self.hiddenLayer.negative_log_likelihood
        )
        # same holds for the function computing the number of errors
        self.errors = self.hiddenLayer.errors
...
def test_mlp(learning_rate=0.01, L1_reg=0.00, L2_reg=0.0001, n_epochs=1000, dataset='mnist.pkl.gz', batch_size=20, n_hidden=500, seed=1234):
   ...
   """
    ...
    ypred = T.dot(train_set_x[0], classifier.params[0]).eval()
    print(ypred)
    print(train_set_y[0].eval())
    print(classifier.params[0].get_value())

    end_time = time.clock()
    with open('../model/bin-to-hex.dat', 'wb') as f:
...
if __name__ == '__main__':
    for seed in range(100,120):
        test_mlp(learning_rate=0.01, L1_reg=0.00, L2_reg=0.0001, n_epochs=1000, dataset='bin-to-hex-interval(0,1000)_length-(50000, 10000, 10000)_seed-100.dat', batch_size=10, n_hidden=10, seed=seed)
