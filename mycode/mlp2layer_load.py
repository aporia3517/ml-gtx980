...
def test_mlp(learning_rate=0.01, L1_reg=0.00, L2_reg=0.0001, n_epochs=1000,
             dataset='mnist.pkl.gz', batch_size=20, hidden_layer_sizes=[500,500], seed=1234, model='../model/2layermodel.dat'):
    """
   ...
   """
    ...
    act3 = T.nnet.softmax(T.dot(act2, classifier.params[0]) + classifier.params[1])
    
    stdev_act1 = T.std(act1, axis=0).eval()
    print(T.min(stdev_act1).eval())
    print(T.max(stdev_act1).eval())

    m_act1 = T.tanh(T.dot(train_set_x, classifier.params[2]) + classifier.params[3]).eval()
    for threshold in range(47, 91, 1):
        threshold /= 1000.
        m_act1[:,stdev_act1 < threshold] = 0
        m_act2 = T.tanh(T.dot(m_act1, classifier.params[4]) + classifier.params[5])
        m_act3 = T.nnet.softmax(T.dot(m_act2, classifier.params[0]) + classifier.params[1])
        print('modified model: %.3f %% with threshold %.3f (dropped off %d nodes) in layer 1' % (T.mean(T.neq(T.argmax(m_act3, axis=1), train_set_y)).eval()*100., threshold, len(stdev_act1[stdev_act1 < threshold])))
    print('training set test end')
    '''
    print('-----------------------------------------')
    ...
    #print(act3.eval()[0:5])
    #print(T.argmax(act3.eval()[0:5], axis=1).eval())
    print('original model: %f %%' % ( T.mean(T.neq(T.argmax(act3, axis=1), test_set_y)).eval()*100. ))
    print('----activation1----')
    act1 = T.tanh(T.dot(valid_set_x, classifier.params[2]) + classifier.params[3])
    act2 = T.tanh(T.dot(act1, classifier.params[4]) + classifier.params[5])
    act3 = T.nnet.softmax(T.dot(act2, classifier.params[0]) + classifier.params[1])
    ...
    for threshold in range(47, 91, 1):
        ...
        m_act2 = T.tanh(T.dot(m_act1, classifier.params[4]) + classifier.params[5])
        m_act3 = T.nnet.softmax(T.dot(m_act2, classifier.params[0]) + classifier.params[1])
        print('modified model: %.3f %% with threshold %.3f (dropped off %d nodes) in layer 1' % (T.mean(T.neq(T.argmax(m_act3, axis=1), test_set_y)).eval()*100., threshold, len(stdev_act1[stdev_act1 < threshold])))
    ...
    for threshold in range(136, 251, 3):
        threshold /= 1000.
        m_act2[:,stdev_act2 < threshold] = 0
        m_act3 = T.nnet.softmax(T.dot(m_act2, classifier.params[0]) + classifier.params[1])
        print('modified model: %.3f %% with threshold %.3f (dropped off %d nodes) in layer 1' % (T.mean(T.neq(T.argmax(m_act3, axis=1), test_set_y)).eval()*100., threshold, len(stdev_act2[stdev_act2 < threshold])))
